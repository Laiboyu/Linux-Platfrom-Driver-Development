## 中斷（Interrupt）
接下會進行中斷操作的介紹以及實作範例說明，當硬體（或軟體）發生事件時，會觸發中斷（Interrupt）機制，由硬體主動去通知 CPU 的事件，轉去處理更緊急事件。
發生中斷後會跳去執行「中斷服務程式（ISR）」的機制。

| 類型                            | 英文名稱         | 觸發時機               | 範例              |
| ----------------------------- | ------------ | ------------------ | --------------- |
| **硬體中斷 (Hardware Interrupt)** | Hardware IRQ | 由外部裝置透過中斷線觸發       | GPIO, USB, UART |
| **軟體中斷 (Software Interrupt)** | SoftIRQ      | Kernel 內部使用，用於延遲任務 | 網路封包處理、tasklet  |
